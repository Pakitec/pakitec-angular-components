...
- Pai: `PLAN_STARTED`, `PLAN_BLOCKED`, inicio do build, `QA_STARTED`, `CODE_REVIEW_STARTED`, `CODE_REVIEW_COMPLETED`, `BUILD_BLOCKED` e `BUILD_COMPLETED`.
- Code review: revisao final consultiva pos-`QA_PASSED`, via Open Code Review em modo delegation. Registra achados por severidade na issue pai e nunca bloqueia `BUILD_COMPLETED` nem dispara ciclo de correcao; achados `critical`/`high` sao alertas para analise humana.
