...
`sdd-spec-writer`, `sdd-researcher`, `sdd-planner`, `sdd-jira-coordinator`, `sdd-implementer`, `sdd-qa-reviewer` e `sdd-code-review`. Passe sempre o nome exato como `subagent_type`. Nunca use `code`, `developer`, `general-purpose` ou fallback direto.
